...
@section('content')
     ...
     @foreach($data as $k=>$v)
     <img src="{{env('UPLOAD_URL')}}{{$v->b_logo}}">
     @endforeach
     </div><!--sliderA/-->
     
     <ul class="pronav">
     @foreach($data as $k=>$v)
      <li><a href="{{url('product/'.$v->g_id)}}">{{$v->b_name}}</a></li>
      @endforeach
      <div class="clearfix"></div>
     </ul><!--pronav/-->
     ...
     @foreach($data as $k=>$v)
      <div class="index-pro1-list">
       <dl>
        <dt><a href="{{url('index/product/'.$v->g_id)}}"> <img src="{{env('UPLOAD_URL')}}{{$v->g_img}}"></a></dt>
        <dd class="ip-text"><a href="{{url('index/product/'.$v->g_id)}}">{{$v->g_name}}</a><span>已售：{{$v->g_num}}</span></dd>
